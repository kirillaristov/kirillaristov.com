...
//отрисовка меню
function openMenu () {
	var menuButton = document.querySelectorAll('.menu__button')[0];
	var menuContent = document.querySelectorAll('.menu')[0];
...
/* maybe to delete */ while(slider.firstChild) {
    ...
    function loadImage(index, callback) {
		...
		if (imageSrc.search(/N\d+-\d+E\d+-\d+/) != -1) {
			...
			imageCaption += '<p class="small coordinates"><a target="_blank" href="/map/#type=hybrid&amp;zoom=12&amp;center=' + coord + '&amp;placemark=' + coord + '">' + coord + '</a></p>';
		}		
...
function ready() {
    ...
    if (document.querySelectorAll('.menu__button').length > 0) {
		openMenu();
	}
